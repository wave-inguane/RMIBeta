...
public class Calendar extends UnicastRemoteObject implements RemCalendar {
	...
	private Map<String, ArrayList<Event>> userCalendar = new TreeMap<String, ArrayList<Event>>(); // calendar for the current user
	private Map<String, Map<String, ArrayList<Event>>> createdBy = new TreeMap<String, Map<String, ArrayList<Event>>>(); // tracks the calendar that was created by a certain user
	...
	public void retrieveMessage(String message) throws RemoteException {
		//Do Nothing call client's retrieveMessage
	}
	...
	public boolean addEvent(String userName,
		...
		if (userCalendar != null) {
			...
			// Gather all the times of the existing user's calendars into lists
			for (Iterator<Map.Entry<String, ArrayList<Event>>> iterator = userCalendar.entrySet().iterator(); iterator.hasNext(); ) {
				...
				if (key.equals(userName) && !accessControl.equals("Group")) {
	...
	public String viewAnyCalendar(String name) throws RemoteException {
		StringBuilder sb = new StringBuilder();
		if (userName.equals(name)){ //isOwner(name) == true) && 
	...
	// Post event in any user's calendar
	...
	private boolean isOwner(String name) throws RemoteException {
		...
		for (Iterator<Map.Entry<String, Map<String, ArrayList<Event>>>> iterator = createdBy.entrySet().iterator(); iterator.hasNext(); ) {
			...
			if (calendar != null) {
				for (Iterator<Map.Entry<String, ArrayList<Event>>> iterator2 = calendar.entrySet().iterator(); iterator2.hasNext(); ) {
					...
					if (((key.substring(0, key2.length())).equals(key2)) && key2.equals(name)){  //is the owner
					    //generalAccess = entry2.getValue();
						return true;
					}
				}
			}
